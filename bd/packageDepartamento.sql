...
AS
    ...
    AS
        V_ID NUMBER;
    BEGIN
        INSERT INTO DEPARTAMENTO (ID,NOMBRE,NUMERO_BANNO,NUMERO_HABITACION,DIRECCION,VALOR_ARRIENDO,ESTADO,ID_LOCALIDAD,DESCRIPCION,ESTADO_DISPONIBLE,ESTADO_RESERVA)
        VALUES(DEPARTAMENTO_AUTO.NEXTVAL,V_NOMBRE,V_NUMERO_BANNO,V_NUMERO_HABITACION,V_DIRECCION,V_VALOR,V_ESTADO,V_LOCALIDAD,V_DESCRIPCION,V_ESTADO_DISPONIBLE,V_ESTADO_RESERVA)
        RETURNING departamento.id INTO V_ID; 
        RESULTADO := V_ID;

        SELECT INVENTARIO_AUTO.NEXTVAL, V_ID AS ID_DEPARTAMENTO, P.ID AS ID_PRODUCTO, P.CANTIDAD_INICIAL FROM PRODUCTO P LEFT JOIN DEPARTAMENTO D ON D.ID IS NULL;
