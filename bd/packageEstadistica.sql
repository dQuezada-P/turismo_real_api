create or replace PACKAGE ESTADISTICA
AS
    PROCEDURE CANT_DEPTOS_DISP (V_CANT_DISP OUT NUMBER);
    PROCEDURE CANT_DEPTOS_RES (V_CANT_RES OUT NUMBER);
    PROCEDURE TOTAL_GANANCIA (V_TOTAL OUT NUMBER);
    PROCEDURE TOTAL_TOUR (V_TOTAL OUT NUMBER);
    PROCEDURE TOTAL_TRANSPORTE (V_TOTAL OUT NUMBER);


    PROCEDURE REPORTE_DEPARTAMENTO (
        V_ID_DEPARTAMENTO IN INT,
        V_FECHA1 IN VARCHAR2,
        V_FECHA2 IN VARCHAR2,
        V_DEPARTAMENTO OUT SYS_REFCURSOR);
        
    PROCEDURE REPORTE_LOCALIDAD (
        V_ID_LOCALIDAD IN INT,
        V_FECHA1 IN VARCHAR2,
        V_FECHA2 IN VARCHAR2,
        V_LOCALIDAD OUT SYS_REFCURSOR);
END;


/
create or replace PACKAGE BODY ESTADISTICA
AS
    PROCEDURE CANT_DEPTOS_DISP (V_CANT_DISP OUT NUMBER)
    AS
    BEGIN
        SELECT COUNT(*) INTO V_CANT_DISP FROM DEPARTAMENTO WHERE ESTADO_RESERVA = 'Y';
    END;
    
    PROCEDURE CANT_DEPTOS_RES (V_CANT_RES OUT NUMBER)
    AS
    BEGIN
        SELECT COUNT(*) INTO V_CANT_RES FROM DEPARTAMENTO WHERE ESTADO_RESERVA = 'N';
    END;
    
    PROCEDURE TOTAL_GANANCIA (V_TOTAL OUT NUMBER)
    AS
    BEGIN
        SELECT SUM(PAGO_TOTAL + PAGO_INCONVENIENTE + ABONO) INTO V_TOTAL  FROM PAGO WHERE ADDED_DATE BETWEEN ADD_MONTHS(CURRENT_DATE, -1) AND CURRENT_DATE;

    END;
    
    PROCEDURE TOTAL_TOUR (V_TOTAL OUT NUMBER)
    AS
    BEGIN
        SELECT COUNT(*) INTO V_TOTAL FROM TOUR;
    END;
    
    PROCEDURE TOTAL_TRANSPORTE (V_TOTAL OUT NUMBER)
    AS
    BEGIN
         SELECT COUNT(*) INTO V_TOTAL FROM TRANSPORTE;
    END;
    
    PROCEDURE REPORTE_DEPARTAMENTO (
        V_ID_DEPARTAMENTO IN INT,
        V_FECHA1 IN VARCHAR2,
        V_FECHA2 IN VARCHAR2,
        V_DEPARTAMENTO OUT SYS_REFCURSOR)
    AS
        BEGIN
            OPEN V_DEPARTAMENTO FOR SELECT 
                D.ID,
                D.NOMBRE,
                R.FECHA_INICIO AS "FECHA",
                R.DIAS,
                R.CANTIDAD_PERSONA,
                SUM(P.ABONO + P.PAGO_TOTAL + P.PAGO_INCONVENIENTE) AS TOTAL,
                L.NOMBRE AS LOCALIDAD
                FROM DEPARTAMENTO D
                JOIN 
                    RESERVA R ON D.ID = R.ID_DEPARTAMENTO 
                JOIN 
                    LOCALIDAD L ON D.ID_LOCALIDAD = L.ID
                JOIN
                    PAGO P ON R.ID = P.ID
                WHERE 
                    TO_DATE(R.FECHA_INICIO, 'DD/MM/YYYY') BETWEEN  TO_DATE(V_FECHA1, 'DD/MM/YYYY') AND  TO_DATE(V_FECHA2, 'DD/MM/YYYY')
                    AND D.ID = V_ID_DEPARTAMENTO
                    AND R.ESTADO BETWEEN 2 AND 3
                GROUP BY D.ID, D.NOMBRE, R.FECHA_INICIO, R.DIAS, R.CANTIDAD_PERSONA, L.NOMBRE     
                ORDER BY 
                    R.FECHA_INICIO ASC;
    END;
    PROCEDURE REPORTE_LOCALIDAD (
        V_ID_LOCALIDAD IN INT,
        V_FECHA1 IN VARCHAR2,
        V_FECHA2 IN VARCHAR2,
        V_LOCALIDAD OUT SYS_REFCURSOR)
    AS
        BEGIN
            OPEN V_LOCALIDAD FOR SELECT 
                D.NOMBRE,
                SUM(P.ABONO + P.PAGO_TOTAL + P.PAGO_INCONVENIENTE) AS TOTAL,
                R.FECHA_INICIO AS "FECHA",
                L.NOMBRE AS LOCALIDAD
                FROM DEPARTAMENTO D
                JOIN 
                    RESERVA R ON D.ID = R.ID_DEPARTAMENTO 
                JOIN 
                    LOCALIDAD L ON D.ID_LOCALIDAD = L.ID
                JOIN
                    PAGO P ON R.ID = P.ID
                WHERE 
                    TO_DATE(R.FECHA_INICIO, 'DD/MM/YYYY') BETWEEN  TO_DATE(V_FECHA1, 'DD/MM/YYYY') AND  TO_DATE(V_FECHA2, 'DD/MM/YYYY')                     
                    AND L.ID = V_ID_LOCALIDAD                 
                    AND R.ESTADO BETWEEN 2 AND 3
                GROUP BY D.NOMBRE, L.NOMBRE, R.FECHA_INICIO
                ORDER BY 
                    R.FECHA_INICIO ASC;
    END;
    
END;